...
from .models import Artist, ListeningEvent, ProfileSnapshot, RawImportRecord, Track, UserTrackRelationship
...
def refresh_relationships(db: Session, user_id: UUID) -> list[UserTrackRelationship]:
    ...
    for track_id in all_track_ids:
        ...
        relation.excluded = not bool(events)
        ...
        relation.discovery_source = next((event.context.get("source") for event in reversed(events) if event.context.get("source")), None)
...
def profile_features(db: Session, user_id: UUID, profile_type: str, as_of: datetime | None = None) -> tuple[dict, datetime | None, datetime | None]:
    ...
    features = {"schema_version": 1, "profile_type": profile_type, "summary": {"weighted_plays": round(total_weight, 4), "unique_tracks": len(seen_tracks), "unique_artists": len(artist_scores), "favorites": favorites, "completion_rate": round(completed / max(1, sum(1 for event, _, _ in rows if aware(event.played_at) and aware(event.played_at) >= window_start)), 4), "skip_rate": round(skips / max(1, sum(1 for event, _, _ in rows if aware(event.played_at) and aware(event.played_at) >= window_start)), 4), "replays": recent_replays}, "artists": artists, "genres": genres, "parameters": {"recent_window_days": RECENT_WINDOW_DAYS, "recency_half_life_days": RECENCY_HALF_LIFE_DAYS}}
