...
def list_imports(db: Session = Depends(get_db)) -> list[dict]:
    return [serialize_batch(batch) for batch in db.scalars(select(ImportBatch).order_by(ImportBatch.created_at.desc())).all()]
...
def review_records(db: Session = Depends(get_db)) -> list[dict]:
    records = list(db.scalars(select(RawImportRecord).where(RawImportRecord.resolution_status.in_(["ambiguous", "unresolved"]), RawImportRecord.excluded.is_(False)).order_by(RawImportRecord.observed_at.desc().nullslast())))
...
def get_profile(user_id: UUID = DEFAULT_USER_ID, as_of: datetime | None = None, db: Session = Depends(get_db)) -> dict:
    ...
    return {profile_type: snapshot.features | {"window_start": snapshot.window_start, "window_end": snapshot.window_end, "calculated_at": snapshot.calculated_at} for profile_type, snapshot in snapshots.items()}
...
def get_timeline_period(period_id: UUID, db: Session = Depends(get_db)) -> dict:
...
def validate_memory_target(db: Session, target_type: str, target_id: UUID) -> None:
    target_models = {"track": Track, "artist": Artist, "album": Album, "timeline_period": TimelinePeriod}
    if not db.get(target_models[target_type], target_id):
...
def create_memory(request: MemoryRequest, user_id: UUID = DEFAULT_USER_ID, db: Session = Depends(get_db)) -> dict:
    validate_memory_target(db, request.target_type, request.target_id)
