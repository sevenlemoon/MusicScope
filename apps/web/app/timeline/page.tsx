...
export default function TimelinePage() {
  ...
  const load = async () => { setLoading(true); setError(false); try { const response = await fetch(`${API_URL}/api/v1/timeline`); if (!response.ok) throw new Error("timeline load failed"); setPeriods(await response.json()); } catch { setError(true); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, []);
  const recalculate = async () => { setMessage(t("timeline.recalculating")); try { const response = await fetch(`${API_URL}/api/v1/timeline/recalculate`, { method: "POST" }); if (!response.ok) throw new Error("timeline recalculation failed"); setPeriods(await response.json()); setMessage(t("timeline.updated")); } catch { setMessage(t("status.error")); } };
