...
export default function PlaylistImport({ onImported }: { onImported?: () => void }) {
  ...
  return <section className="panel playlist-import"><div className="section-heading"><div><p className="eyebrow">{t("home.heroKicker")}</p><h2>{t("home.importTitle")}</h2><p className="muted">{t("home.importDescription")}</p></div></div><ol className="import-steps"><li><strong>1</strong><span>{t("playlist.step1")}</span></li><li><strong>2</strong><span>{t("playlist.step2")}</span></li><li><strong>3</strong><span>{t("playlist.step3")}</span></li></ol><div className="import-methods"><button type="button" className={method === "text" ? "selected-role" : "secondary-button"} onClick={() => setMethod("text")}>{t("playlist.textTab")}</button><button type="button" className={method === "csv" ? "selected-role" : "secondary-button"} onClick={() => setMethod("csv")}>{t("playlist.otherMethods")}</button></div>{method === "text" ? <><div className="playlist-input-row"><input aria-label={t("playlist.placeholder")} value={url} onChange={(event) => setUrl(event.target.value)} placeholder={t("playlist.placeholder")} /><button type="button" onClick={() => void inspect()} disabled={checking}>{t("playlist.detect")}</button></div><p className="muted playlist-help">{t("playlist.textHelp")}</p><a className="text-link" href="https://music.unmeta.cn/" target="_blank" rel="noreferrer">{t("playlist.openTool")}</a><textarea aria-label={t("playlist.textPlaceholder")} className="playlist-textarea" value={text} onChange={(event) => setText(event.target.value)} placeholder={t("playlist.textPlaceholder")} /><p className="muted">{t("playlist.summary", { total: parsed.summary.totalLines, valid: parsed.summary.validTracks, invalid: parsed.summary.invalidLines, artists: parsed.summary.uniqueArtists })}</p><button type="button" onClick={() => void importText()} disabled={checking || !parsed.rows.length}>{t("playlist.parse")}</button><p className="muted attribution">{t("playlist.attribution")}</p><p className="import-note">{t("playlist.membershipNote")}</p></> : <label className="secondary-upload">{t("playlist.csv")}<input type="file" accept=".csv,text/csv" onChange={importCsv} /></label>}<p className="import-status" role="status">{message}</p></section>;
}
